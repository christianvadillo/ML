...
Scratch:
    Artificial Neural Network using softmax for multiclass on e-comerce data
"""
...
# for multiclass
# =============================================================================
M = 5  # Number of neurons in a layer
...
for epoch in range(epochs):
    # feedforward (X_train)
    Z_train = np.tanh(X_train.dot(W1) + b1)  # layer output
    A = Z_train.dot(W2) + b2
    # softmax to 'a_train'
    output_train = np.exp(A) / np.exp(A).sum(axis=1, keepdims=True)

    # fedforward (X_test)
    Z_test = np.tanh(X_test.dot(W1) + b1)  # layer output
    A = Z_test.dot(W2) + b2
    # softmax to 'a_train'
    output_test = np.exp(A) / np.exp(A).sum(axis=1, keepdims=True)
    ...
    # =============================================================================
    # Updating parameters using Gradient Descent and backward propragation
    # =============================================================================
    W2 -= learning_rate * Z_train.T.dot(output_train - y_train)
    b2 -= learning_rate * (output_train - y_train).sum(axis=0)
    # Error at hidden nodes
    dZ = (output_train - y_train).dot(W2.T) * (1 - Z_train * Z_train) 
